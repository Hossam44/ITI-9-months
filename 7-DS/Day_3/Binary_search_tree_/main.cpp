...
public:
    ...
    void traverse_all_nodes(node* proot,node* arr[],int &counter)
    {
        if(proot!=nullptr)
        {
            traverse_all_nodes(proot->left,arr,counter);
            arr[counter++]=proot;
            traverse_all_nodes(proot->right,arr,counter);
        }
    }
    ...
    node* Search(node* proot,int k)
    {
        if(proot!=nullptr)
        {
            ...
            {
                return Search(proot->left,k);
            }
            else
            {
                return Search(proot->right,k);
            }
        }
        return nullptr;
    }
...
{
    node *pnew = B1.new_node(temp);
    B1.Delete(B1.Troot,temp.id);
    B1.insert_(B1.Troot,pnew);
...
void Enter_Data()
{
    ...
    do{
        ...
        textattr(NormalPen);
        gotoxy(0,0);
        if(B1.Search(B1.Troot,temp.id)!=nullptr)
        {
            f=0;
            getch();
...
void Show_Data_of_all_employee(){
    ...
    int num_nodes = B1.count_nodes(B1.Troot),counter=0;
    node* arr[num_nodes];
    B1.traverse_all_nodes(B1.Troot,arr,counter);

    char ch,current=0,ex=1;
    counter = 0;
    diplay_data_of_i_emplyee_small_info(arr[0]);
                    ...
                    /*
                        ...
                        */
                        if(counter>0)
                            ...
                            counter = num_nodes-1;
                        diplay_data_of_i_emplyee_small_info(arr[counter]);
                        break;
                    }
                case 1:
                    {
                        display_full_info(arr[counter]);
                        ch=getch();
                        system("cls");
                        diplay_data_of_i_emplyee_small_info(arr[counter]);
                        print_small_structure_info(1);
...
void delete_by_name()
{
    ...
    int num_nodes = B1.count_nodes(B1.Troot),counter=0,flag=0;
    node* arr[num_nodes];
    B1.traverse_all_nodes(B1.Troot,arr,counter);

    for(int i=0;i<num_nodes;i++)
    {
        if(!strcmp(arr[counter++]->n.name,name))
        {
            flag = 1;
            break;
        }
    ...
    if(flag)
        B1.Delete(B1.Troot,arr[counter-1]->n.id);
}
