...
{
    S1.Delete_From_linked_listby_ID(change,temp.id);
    S1.Push_elemnt(temp);
}
...
void Enter_Data()
{
    ...
    do{
        ...
        textattr(NormalPen);
        gotoxy(0,0);
        if(S1.Search(temp.id)!=nullptr)   //to check there same id
        {
            f=0;
            getch();
