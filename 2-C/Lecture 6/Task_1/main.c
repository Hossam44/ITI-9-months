...
void set_data_in_array(struct emp temp)
{
    for(int i=0;i<=100;i++)
    {
        if(e[i].id==-1)
        {
            ...
            strcpy(e[i].Gen,temp.Gen);                                      //we replace it becuse error of gender array
...
void Line_editor(char *arr,int xpos,int ypos,char st,char en)
{
    char ch,temp;
    int all_elment=0,arrow=0;

    do{
        gotoxy(arrow+xpos,ypos);
        ch=getch();
        //system("cls");


        if(ch>=st && ch<=en && all_elment <= 10)
        {
            all_elment++;
            for(int i=10;i>arrow;i--)
            {
                char temp = arr[i];
                arr[i]=arr[i-1];
                arr[i-1]=temp;
...
{
    char arr[12];
    Line_editor(arr,37,17,'.','9');
    temp.salary = atof(arr);
}

void Enter_address()
{
    char arr[12];
    Line_editor(arr,37,21,'a','z');
    strcpy(temp.address,arr);
}

void Enter_age()
{
    char arr[12];
    Line_editor(arr,77,9,'0','9');
    temp.age = atoi(arr);
}
...
{
    char arr[12];
    Line_editor(arr,77,21,'.','9');
    temp.over_tm = atof(arr);
}
